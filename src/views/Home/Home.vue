  ...
  <section>
   <van-collapse v-for="(item,index) in dayComputed.habits" :key="item.id" @change="change" v-model="today.active">
    <van-collapse-item :name="index">
     <p slot="title">
      <icon name="time"/>{{item.title}}
     </p>
     <aside v-for="ele in item.habits" :key="ele.id" @click="finish(ele.id)">
      <Circles radius="3.5rem" v-if="!!ele.habitLog.date.find(item=>(item.id===days))"
       :activeColor="!!ele.habitLog.date.find(item=>(item.id===days)).isFinished ? ele.color :'#fff'">
...
<script lang="ts">
 ...
 export default class Today extends Vue {
  ...
  @Mutation private changeFinished!: (playload: {
   id: number;daysId: number
  }) => void;
  @Mutation private changeCollapse!: (habit: number[] | never[]) => void;
  @Mutation private updateHabits!: (updateList: number[]) => void;
  @Mutation private saveLog!: (playload: {
   id: number;daysId: number;message: string
  }) => void;
  ...
  private mounted() {
   ...
   console.log(this.dayComputed,"needUpdate")
  ...
  private get dayComputed() {
   const habitList = this.habitList.filter((item) => item.isActive === true);
   // 今天可用的习惯
    ...
    for (let i = 0; i < current.length; i++) {
     ...
     if (time!.title === element) {
      if (newList.title === time!.title) {
       console.log(newList,time,'1')
       // @ts-ignore
       newList.habits.push(item);
      } else {
       newList.title = element;
       newList.id = index;
        console.log(newList,time,'2')
       // @ts-ignore
       newList.habits.push(item);
  ...
  private finish(id: number) {
   ...
   if (this.habitList.find((item) => item.id === id) !.habitLog.date.find((item) => item.id === this.days) !
    .isFinished) {
